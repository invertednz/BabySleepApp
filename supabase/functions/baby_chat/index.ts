...
function classifyQuestion(message: string): string[] {
  ...
  if (/milestone|develop|crawl|walk|talk|sit|stand|roll|grab|point|wave|clap|first|learn|skill/.test(lowerMsg)) {
    categories.push('milestones');
  }
  if (/sleep|nap|bedtime|wake|night|tired|rest|drowsy|schedule|routine/.test(lowerMsg)) {
    categories.push('sleep');
  }
  if (/play|activity|game|toy|like|hate|love|fun|bored|engage|stimulat|entertain/.test(lowerMsg)) {
    categories.push('activities');
  }
  if (/eat|feed|food|bottle|breast|formula|solid|wean|hungry|milk|nursing|meal|snack/.test(lowerMsg)) {
    categories.push('feeding');
  }
  if (/sick|health|doctor|concern|worry|fever|rash|pain|cry|teething|diaper|poop|pee|wet/.test(lowerMsg)) {
    categories.push('health');
  }
  if (/word|say|speak|language|vocab|babbl|sound|communicate/.test(lowerMsg)) {
    categories.push('vocabulary');
  }
  if (/advice|plan|focus|recommend|should|week|help|tips|suggest|best|normal|priority|goal/.test(lowerMsg)) {
...
Deno.serve(async (req) => {
  try {
    ...
    const { data: userPrefs } = await supabase
      ...
      .select('plan_tier, is_on_trial, parenting_styles, nurture_priorities, goals')
      .eq('user_id', userId)
      .maybeSingle();
    ...
    if (focus.length > 0 || priorities.length > 0 || parentGoals.length > 0) {
      ...
      if (priorities.length > 0) priorityText += ` Priorities: ${priorities.join(', ')}.`;
      if (parentGoals.length > 0) priorityText += ` Goals: ${parentGoals.join(', ')}.`;
      if (parentingStyles.length > 0) priorityText += ` Style: ${parentingStyles.join(', ')}.`;
    ...
    if (milestones && milestones.length > 0) {
      ...
      if (inWindow.length > 0) msText += ` Currently working on: ${inWindow.slice(0, 5).map((m: any) => m.title).join(', ')}.`;
      if (overdue.length > 0) msText += ` May need attention: ${overdue.map((m: any) => m.title).join(', ')}.`;
      if (achieved.length > 0) msText += ` Achieved ${achieved.length} milestones.`;
    ...
    if (categories.includes('sleep')) {
      ...
      if (sleepData && sleepData.length > 0) {
        ...
        if (s.naps && Array.isArray(s.naps)) sleepText += ` ${s.naps.length} naps.`;
    ...
    if (categories.includes('health')) {
      ...
      if (concerns && concerns.length > 0) {
        contextParts.push(`Active concerns: ${concerns.map((c: any) => c.text).join('; ')}`);
      }
    }
    ...
    if (categories.includes('vocabulary')) {
      ...
      if (vocabData && vocabData.length > 0) {
        contextParts.push(`Vocabulary (${vocabData.length} recent words): ${vocabData.map((v: any) => v.word).join(', ')}`);
      }
    }

    // 9. CURRENT WEEKLY PLAN (if valid)
    if (categories.includes('development') || categories.includes('activities')) {
      ...
      if (weeklyAdvice?.plan?.weekly_summary?.theme) {
        contextParts.push(`This week's focus: ${weeklyAdvice.plan.weekly_summary.theme}`);
      }
    }
...
- Don't diagnose conditions`;
    ...
    const contents: any[] = [
      ...
      { role: 'model', parts: [{ text: `I'm here to help with ${baby.name}. What would you like to know?` }] },
    ];
